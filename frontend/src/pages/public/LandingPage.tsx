...
const pricing: Pricing[] = [
  {
    ...
    price: 'Gratis',
    period: 'selamanya',
    description: 'Untuk RT/RW kecil yang baru memulai digitalisasi.',
    features: ['S/d 50 warga', 'Iuran manual', 'Forum dasar', '1 pengurus'],
    cta: 'Mulai Gratis',
  },
  {
    name: 'Professional',
    price: 'Rp 99K',
    period: '/bulan',
    description: 'Untuk RT/RW aktif yang butuh otomatisasi penuh.',
    features: ['Warga unlimited', 'Iuran QRIS otomatis', 'Forum + lapak', 'Hingga 5 pengurus', 'Dashboard analitik', 'Priority support'],
  ...
  },
  {
    ...
    description: 'Untuk kelurahan atau cluster dengan kebutuhan khusus.',
    features: ['Multi-tenant', 'Xendit sub-account', 'SSO & API access', 'SLA 99.9%', 'Dedicated support'],
    cta: 'Hubungi Sales',
  },
]
...
const faqs: Faq[] = [
  ...
  { q: 'Apakah bisa coba dulu sebelum bayar?', a: 'Ya! Plan Professional bisa dicoba gratis selama 14 hari. Tidak perlu kartu kredit — daftar langsung pakai nomor WA.' },
  { q: 'Bagaimana sistem pembayaran iuran?', a: 'Menggunakan Xendit QRIS — warga scan QR bayar via DANA, OVO, GoPay, atau ShopeePay. Dana masuk ke rekening tenant (untuk Enterprise) atau rekening Logikraf (untuk Starter/Professional).' },
    ...
    <div className="min-h-screen bg-surface text-text-primary">
      ...
      <nav className="sticky top-0 z-50 border-b border-border bg-surface/80 backdrop-blur-md">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            <Link to="/login" className="rounded-lg px-4 py-2 text-sm font-medium text-text-secondary transition-colors hover:bg-text-disabled/10 hover:text-text-primary">
              Masuk
            </Link>
            <Link to="/register-pengurus" className="rounded-lg bg-primary px-4 py-2 text-sm font-medium text-white transition-all hover:bg-primary/90 active:scale-95">
              Daftar Gratis
            </Link>
          </div>
        </div>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-6xl px-4 pb-20 pt-16 text-center md:pb-28 md:pt-24">
          ...
          <p className="mx-auto mt-6 max-w-2xl text-lg text-text-secondary md:text-xl">
            Iuran otomatis via QRIS, forum warga real-time, lapak internal, dan dashboard analitik — semua dalam satu aplikasi ringan yang bisa diakses siapa saja.
          </p>
          <div className="mt-10 flex flex-col items-center justify-center gap-3 sm:flex-row">
            <Link to="/register-pengurus" className="inline-flex w-full items-center justify-center gap-2 rounded-xl bg-primary px-8 py-3.5 text-sm font-semibold text-white shadow-lg shadow-primary/25 transition-all hover:bg-primary/90 hover:shadow-xl hover:shadow-primary/30 active:scale-[0.98] sm:w-auto">
      ...
      <section id="harga" className="py-20">
        <div className="mx-auto max-w-6xl px-4">
          <div className="text-center">
            ...
            <p className="mt-4 text-lg text-text-secondary">Mulai gratis kapan saja. Upgrade hanya jika sudah siap.</p>
          </div>
          <div className="mt-14 grid gap-6 lg:grid-cols-3">
            {pricing.map((p, i) => (
              <div key={i} className={`relative rounded-2xl border p-6 transition-all ${p.highlight ? 'border-primary bg-primary/5 shadow-xl shadow-primary/10 scale-[1.02]' : 'border-border bg-surface hover:border-primary/30 hover:shadow-lg'}`}>
                ...
                <Link
                  to={p.name === 'Enterprise' ? '/contact' : '/register-pengurus'}
                  className={`mt-8 block w-full rounded-xl py-3 text-center text-sm font-semibold transition-all active:scale-[0.98] ${p.highlight ? 'bg-primary text-white hover:bg-primary/90' : 'border border-border bg-surface-card text-text-primary hover:bg-text-disabled/5'}`}
                >
                  {p.cta}
                </Link>
              </div>
      ...
      <section className="py-20">
        <div className="mx-auto max-w-4xl px-4 text-center">
          ...
          <p className="mt-4 text-lg text-text-secondary">Bergabung dengan ratusan RT/RW yang sudah menggunakan SmartHub untuk mempermudah manajemen warga.</p>
