...
export function ForumFeed({ canAnnounce, basePath }: Props) {
  ...
  return (
    <>
      {/* Top row: tagline kiri, tombol buat + lonceng kanan ala Threads */}
      <div className="flex items-center justify-between gap-3 py-2.5">
        <p className="min-w-0 flex-1 truncate text-[13px] text-text-secondary">
          Ayo ngobrol &amp; berbagi info dengan tetangga
        </p>
        <div className="flex items-center gap-1">
          <button
            ...
            className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-primary text-white shadow-sm transition-transform hover:scale-105 active:scale-95"
          >
          ...
          <button
            ...
            className="relative grid h-9 w-9 shrink-0 place-items-center rounded-full text-text-secondary transition-colors hover:bg-text-disabled/10 active:scale-95"
          >
      ...
      {err && (
        <div className="mt-1 flex items-center gap-2.5 rounded-xl border border-border/70 bg-surface-card px-3.5 py-3 text-sm text-danger">
          <Icon name="alert" size={16} className="shrink-0" />
          <span className="min-w-0 flex-1">{err}</span>
          <button type="button" onClick={() => void muat()} className="shrink-0 font-semibold underline">
            Muat ulang
          </button>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="px-6 py-14 text-center">
          ...
          <p className="mt-3 font-semibold text-text-primary">Belum ada diskusi</p>
          <p className="mt-1 text-sm text-text-secondary">Jadilah yang pertama mengobrol dengan tetangga.</p>
        </div>
      ) : (
        /* Daftar thread: baris bersih tanpa kartu, hanya pembatas bawah tipis */
          ...
          {daftar.map((t) => (
            <li key={t.id_thread}>
              <button
                ...
                className="w-full py-3.5 text-left transition-colors hover:bg-text-disabled/5 active:bg-text-disabled/10"
              >
                <div className="flex items-start gap-3">
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="flex items-center gap-1.5">
                      <span className="truncate text-[15px] font-semibold leading-tight text-text-primary">{t._penulis}</span>
                      <span className="shrink-0 text-xs text-text-secondary">· {timeAgo(t.created_at)}</span>
                    ...
                    <h2 className="mt-0.5 text-[15px] font-bold leading-snug text-text-primary">{t.judul}</h2>
                    <p className="mt-0.5 line-clamp-2 text-[14px] leading-snug text-text-secondary">{renderRich(t.konten)}</p>
                    {t.foto_url && /^(https?:\/\/|\/)/.test(t.foto_url) && (
                      <img src={t.foto_url} alt="" loading="lazy" className="mt-2 max-h-72 w-full rounded-xl object-cover" />
                    )}
                    <span className="mt-2 flex items-center gap-1 text-xs text-text-secondary">
                      <Icon name="chat" size={12} /> {t.komentar_count ?? 0}
                    </span>
                  </span>
                </div>
              </button>
      ...
      <Drawer open={openBuat} onClose={() => setOpenBuat(false)} title="Buat postingan" subtitle="Bagikan kabar, tanya, atau info ke warga">
        <div className="space-y-4">
          ...
          <button
            ...
            disabled={kirim}
            onClick={kirimThread}
            className="h-12 w-full rounded-full bg-primary font-semibold text-white transition active:scale-[0.98] disabled:opacity-50"
          >
